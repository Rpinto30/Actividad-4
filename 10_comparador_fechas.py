...
def days_between_months(d_minus,d_mayor):
    total = 0
    for i in range(d_minus[1]+1, d_mayor[1]):
        print(i)
        total += month_true(i,d_mayor[2])
    print(days_on_month(d_minus), total)
...
if len(dates) > 0:
    ...
    if dates[0][2] > dates[1][2]: #FECHA 1 MAYOR QUE FECHA 2
        ...
        if dates[0][1] > dates[1][1]: #FECHA 1 MAYOR QUE FECHA 2
            mayor_date(dates[0], dates[1])
            days_between += days_between_months(dates[1], dates[0])
        elif dates[0][1] < dates[1][1]: #FECHA 2 MAYOR QUE FECHA 1
            mayor_date(dates[1], dates[0])
            days_between += days_between_months(dates[0], dates[1])
        else: #MESES IGUALES
            if dates[0][0] == dates[1][0]: print("Ambas fechas son iguales")
            else:
                days_between += abs(dates[0][0] - dates[1][0]) +1
